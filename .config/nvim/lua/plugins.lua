...
local plugins = {
...

	{
		...
		build = ":TSUpdate",
		config = function()
			-- 1) Make Neovim recognize *.bicepparam as filetype=bicepparam
			vim.filetype.add({
				extension = {
			...
			-- 2) Reuse the bicep Treesitter grammar for bicepparam
			pcall(function()
			...
			configs.setup({
				...
				-- Turn on treesitter-based indentation (python exception just there for future reference)
